...
\noindent\albert{Diego: Esta nota en azul que sigue está modificada, inlcuíla así en la versión que mandes a Michigan, y borrá esta frase naranja :)}
...
\noindent
\albert{where $K$ is the number EUV bands, and $\TRF^{(k)}$ is the \emph{temperature response function} of the $j$-th detector. The TRFs are here computed based on the (known) channel’s passband times the coronal emissivity at that temperature (normalized by the squared electron density). The emissivity model used here is provided by the latest version of the CHIANTI atomic database and plasma emission model \citep{delzanna_2015,landi_2013}.}

Due to unresolved coronal dynamics, tomographic reconstructions exhibit negative values of the reconstructed FBE, or zero when the solution is constrained to positive values \citep{frazin_2000,frazin_2009}. These non-reconstructed voxels are indicated \albert{in black color in the latitude-longitude (Carrington) maps of DEMT} reults in Section \ref{resu}.


Once the LDEM is determined at each voxel, \albert{the LDEM-averaged squared electron density $\Nm^2$ and electron temperature $\Tm$} in the voxel can be computed by taking its zeroth and first moments over temperature. More specifically, at the $i$-th voxel,
...
\noindent 
\albert{being the} average relative difference between the tomographic and the synthetic FBEs. \albert{The final product of DEMT is in the form of 3D maps of the LDEM-averaged quantities $\sqravgN$ and $\Tm$, as well as of the measure $R$.}\\
...
In Section \ref{resu}, the results of both the DEMT and AWSoM models in the distinct magnetic strucures are statistically analysed. {As shown \albert{in Section \ref{demt_res}} below, classification of legs from type 0 through II allows studying the streamer belt in three distinct, progressively outer layers.}
