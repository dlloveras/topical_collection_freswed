...
%-------------remove these two lines before submision--------------------------
...
\albert{Down loops are to be expected if heating is enhanced at the footpoints of coronal structures. \citet{schiff_2016} reproduced down loops by means of numerical simulations based on a 1D steady-state model, by requiring that the initial population of Alfvén waves is efficiently converted into compressive modes. Mode conversion is favored by the $\beta\gtrsim 1 $ condition found to characterize the regions where down loops are observed in DEMT \citep{nuevo_2013}. In the AWSoM model, heating is controlled by two key parameters, namely, the Alfvén wave dissipation length, and its reflection coefficient. So far, attempts to reproduce down loops within AWSoM simulations by increasing the reflection coefficient have not been successful.}
