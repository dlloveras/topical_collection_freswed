...
\title{Thermodynamic Structure of the Solar Corona: Tomographic Reconstructions and MHD Modeling\\
      \notebyalbert{\small Corregí filiación UNTREF, agregué segunda filiación para Fede (creo que no es preciso poner CBC, Fede?). Diego, ORCID IDs faltantes sacalos de {\tt orcid.org}}}
...
For each open field line, and for each leg of the closed ones, an exponential fit was then applied to \albert{the $\sqravgN(r)$ data points, and a linear fit applied to the $\Tm(r)$ data points}, as described by
...
\subsection{\albert{Energy input flux}}\label{energia} 
...
\caption{Latitude-longitude location of each leg of \albert{loops of type 0 (cyan), I (blue), II (red) and III (green)} at $1.105\,\mrsun$ for CR-2082 (left panel) and CR-2208 (right panel). See Section \ref{trace} for a definition of the different types of \albert{loops}. \notebyalbert{(by ALBERT) The population of DOWN loops in CR-2008 is negligible. These are VERY SMALL loops, I am pretty sure but you and Ceci shoudl check it.}  \notebyalbert{(by ALBERT) This Figure is great, but I do not quite like the choice of colors. I suggest these changes: cyan$\rightarrow$blue, blue$\rightarrow$red, red$\rightarrow$violet. In this way, blue will be DOWN loops and red will be UP loops, as in previous papers. The same change applies to Figure 4. Also, how about making Figures 3 and 4 a single Figure of four panels? Just an idea.} }
\label{rpoint_demt}
\end{center}
\end{figure} 
...
\caption{Frequency histograms of the temperature radial gradient for the different \albert{four types of loops} shown in Figure \ref{rpoint_demt} (using the same color code) for CR-2082 (left panel) and CR-2208 (right panel).}
\label{gradt_demt}
\end{center}
\end{figure} 

\temp{este inicio de linea se repite del anterior}\\
As an example of the traced magnitudes, Figure \ref{histos_fulldemt} shows the statistical results of both rotations for all field lines types. From top to bottom type 0 to type 4. From left to right the panels show the statistical distribution of \albert{$\NCB\equiv\sqravgN(r=1.055\,\mrsun)$}, $\lN$, $\aTm$, in each panel the median $m$ is indicated.
...
\caption{Same as Figure \ref{rpoint_demt}, but using the density and temperature of the AWSoM model to classify \albert{its loops in types I, II and III. The model does not exhibit loops of type 0.}}
\label{rpoint_awsom}
\end{center}
\end{figure} 
...
\caption{\albert{Average fits to $\Ne(r)$ (left panels) and $T_e(r)$ (right panels) for legs of type I (blue), II (red), and III (green), for CR-2082 (top panel) and CR-2208 (bottom panel). Solid lines correspond to DEMT results while dashed lines correspond to AWSoM results.}}
...
\caption{\albert{Longitude-averaged latitudinal dependence of the AWSoM model wind speed at $6\,\mrsun$ for CR2082 (left panel) and CR2208 (right panel).}}
\label{perf_lon_vr}
\end{center}
\end{figure}
