...
Once the LDEM is determined at each voxel, the average {squared electron density $N_e^2$} and the electron mean temperature $T_\textrm{m}$ in the voxel can be computed by taking its zeroth and first moments over temperature. More specifically, at the $i$-th voxel,
\begin{eqnarray}\label{momento1}
 N_{e,i}^2 &=& \int\,\textrm{d}T\,\LDEM_i(T),\label{Ne} \\ 
\label{momento2}
 T_{\textrm{m},i}   &=& \frac{1}{N_{e,i}^2}\,\int\,\textrm{d}T\,T\,\LDEM_i(T),\label{Tm} 
\end{eqnarray}


so that the final product of DEMT is in the form of 3D maps of the electron density and mean temperature. {Once again, these relationships (derived in detail in \citealt{frazin_2009}, see Appendix C) are similar to those in standard DEM analysis, in which the line-of-sight DEM appears instead of the voxel's LDEM, and the EM appears instead of the squared electron density.} 
...
For both rotations, the DEMT AWSoM 3D maps of electron density $N_e$ and electron mean temperature $T_\textrm{m}$ were traced along the magnetic field lines modeled with AWSoM as described at the end of Section \ref{awsom}. In the case of closed loops, results were separated into their two {legs}, defined as the two segments that go from the coronal base up to the apex.
...
In the case of the electron density, the fitted function corresponds to the isothermal hydrostatic equilibrium solution, allowing for variation of the solar gravitational acceleration with height. After computing the footpoint ($r=1.0\,\mrsun$) electron density $N_{0}\,[\cm^{-3}]$ and scale height $\lN\,[\mrsun]$, this choice of function provides a straightforward means to directly evaluate how compatible is the observed coronal thermodynamical state with the hydrostatic solution. In the case of the DEMT electron mean temperature, the linear fit allows characterization of its variation with height by means of a characteristic temperature gradient $a = \textrm{d}T_m/\textrm{d}r\,[\MK/\mrsun]$ along each leg/loop. 

\textcolor{red}{*Nos quedamos con los fiteos que cumplen test de hipotesis chi cuadrado en temp y densidad.
OBS: Esta es la seleccion para las figuras 8 en adelante.}
...
\caption{Latitude-longitude location of each leg of type I (in blue), type II (in red) and type III (in green) meeting the criteria listed in Section \ref{trace} traced with DEMT results, at heights $1.105\,\mrsun$, for CR-2082 (left panel) and CR-2208 (right panel).}
\label{rpoint_demt}
\end{center}
\end{figure} 
...
\caption{Average fitted of $N_e(r)$ and $T_e(r)$ along all field lines of type I (in blue), of type II (in red) and of type III (in green) meeting the criteria listed in Section \ref{trace} for CR-2082(top panel) and CR-2208 (bottom panel). In solid lines DEMT traced results and in dashed lines AWSoM results.}
...
\caption{Longitude-averaged of electron density for DEMT (blue) and AWSoM (red) results at $1.105\,\mrsun$. On the left CR2082 and on the right CR2208, the vertical black lines indicates the longitude-averaged latitudes of the open/close magnetic boundaries.}
\label{perf_lon_ne}
\end{center}
\end{figure}
