...
We used AWSoM results with two purposes. On the one hand to provide thermodynamic results to be compared with those reconstructed by DEMT, and on the other hand to provide MHD coronal magnetic field model that is used to study thermodynamic variables along field lines.
}
...
{We carried out the 3D DEMT reconstruction of two target CR. We selected CR-2082, a deep minimum rotation between SCs 23 and 24, and CR-2208, a rotation during the early declining fase of SC 24. To study CR-2082 and CR-2208, as mentioned in Section \ref{intro}, data taken by the STEREO/EUVI-B and SDO/AIA instruments was used, respectively. The EUVI and AIA data were prepared using the latest processing tools and calibration corrections provided by their teams through the SolarSoft package. For this work, we introduced two improvements in the implementation of the DEMT technique, as described next.}
...
{The high temperature of the corona requires heating mechanisms to compensate for the energy losses. While the vast majority of the existing literature on coronal heating focuses on active regions (ARs), some studies have been dedicated to the heating of quiet-Sun regions. In particular,  \citet{maccormack_2017} developed a new application of the DEMT technique to estimate the energy input flux required at the base of quiet-Sun coronal loops to mantain them stable. The technique is based on tracing the DEMT results along field lines of a global coronal magnetic model, just as described in Section \ref{trace}.}
